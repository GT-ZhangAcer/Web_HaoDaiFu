...
from selenium.webdriver.firefox import check
import sys
import time
...
class proxyc:
    ...
    # 先初始化得到代理
    def __init__(self, proxynum, key):
        try:
        ...
        Check = "GET:www.66ip.cn/ID_KEY"  # 登录代理库验证是否处于会员状态
        self.key=key
        check.check(self.key)
        firefoxOpt = Options()
        firefoxOpt.add_argument("--headless")
        self.driver = webdriver.Firefox(workPath() + 'exe/core/', firefox_options=firefoxOpt)
        self.findapi()
    ...
    def findapi(self):
        ...
        self.driver.get(url)
        self.driver.set_page_load_timeout(5)  # 等待JS加载时间
        page = self.driver.page_source  # 获取当前页面源码
